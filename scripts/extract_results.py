...
if len(sys.argv) < 2:
    print("Usage: python extract_results.py <FL.log>")
    sys.exit()
...
# Get config file
config_match = re.search(r"Config file:\s*(.+)", text)
config = config_match.group(1).strip() if config_match else "Unknown"

# Complete evaluation blocks
pattern = re.compile(
    r"\[Round:\s*(\d+)\].*?"
    r"Evaluate global model's performance.*?"
    r"=> Loss:\s*([0-9.]+).*?"
    r"=> Accuracy:\s*([0-9.]+)%.*?"
    r"=> Precision:\s*([0-9.]+).*?"
    r"=> Recall:\s*([0-9.]+).*?"
...
if not rows:
    print("No complete evaluation rounds found.")
    sys.exit()
...
print(
    f"Best Accuracy : {best_acc['accuracy']:.2f}% "
    f"(Round {best_acc['round']})"
)

print(
    f"Best Precision: {best_precision['precision']:.4f} "
    f"(Round {best_precision['round']})"
)

print(
    f"Best Recall   : {best_recall['recall']:.4f} "
    f"(Round {best_recall['round']})"
)

print(
    f"Best F1       : {best_f1['f1']:.4f} "
    f"(Round {best_f1['round']})"
)

print(
    f"Lowest Loss   : {best_loss['loss']:.4f} "
    f"(Round {best_loss['round']})"
)
...
print(f"Average time/round: {avg_time:.2f} sec")
print(f"Average time/round: {avg_time/60:.2f} min")
